...
class RecoveryAnimation extends StatelessWidget {
  String text='';
  final controller; //variável que receberá o controlador

  RecoveryAnimation({Key? key, this.controller, required this.text})
      : //construtor da classe (recebe o controlador)
        ...
        buttonSqueeze = Tween(
            ...
            .animate(CurvedAnimation(
                parent: controller,
                curve: Interval(
                    0.0, 0.20) //intervalo em que essa animação ocorrerá
                )),
        buttonZoomOut = Tween(
          begin: 60.0, //tamanho em que inicia
            ...
            parent: controller,
            curve: const Interval(0.5, 1, //intervalo da animação
                curve: Curves.bounceOut //curva
                ))),
        super(key: key);

  final Animation<double>
      buttonSqueeze; //animação para encolher largura do botão
            ...
            child: //operador térnario
                buttonZoomOut.value == 60
                    ? Container(
                        ...
                        child: _buildInside(
                            context), //informações de dentro do botão
                        //decoração do botão
                        decoration: const BoxDecoration(
                            color:
                                const Color.fromRGBO(30, 120, 160, 1.0), //cor
                            borderRadius: BorderRadius.all(
                                Radius.circular(30.0)) //borda oval
                            ))
                    : Container(
